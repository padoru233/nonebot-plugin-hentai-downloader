...
async def test_pip(app: App):
    ...
    assert require("nonebot_plugin_hentai_downloader")

    event = make_onebot_msg(Message("pip install nonebot2"))
    try:
        from nonebot_plugin_hentai_downloader import pip
    except ImportError:
        pytest.skip("nonebot_plugin_hentai_downloader.pip not found")
